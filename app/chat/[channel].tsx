...
import { useChatMessages, sendChatMessage } from '@/lib/db';
    ...
    <View style={[styles.bubbleContainer, isOwn ? styles.bubbleRight : styles.bubbleLeft]}>
      {!isOwn && (
        <View style={[styles.avatar, { backgroundColor: channelColor + '20' }]}>
          <Text style={[styles.avatarText, { color: channelColor }]}>
      ...
      >
        {!isOwn && (
          <Text style={[styles.bubbleNickname, { color: channelColor }]}>
            {message.nickname}
          </Text>
        )}
...
export default function ChatScreen() {
  ...
  const { data: messages = [], isLoading } = useChatMessages(channel || 'general');
  ...
  useEffect(() => {
    AsyncStorage.getItem('chat_nickname').then(saved => {
      if (saved) {
        setNickname(saved);
  ...
  const handleSend = useCallback(async () => {
    if (!inputText.trim() || !nickname || isSending) return;
    if (Platform.OS !== 'web') Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);

    const text = filterProfanity(inputText.trim());
    setInputText('');
    setIsSending(true);

    try {
      await sendChatMessage(channel || 'general', nickname, text);
    } catch {
  ...
  }, [inputText, nickname, isSending, channel]);

  const handleReport = () => {
    Alert.alert(
      'Report Message',
      'This message has been flagged for review. Thank you for helping keep our community safe.',
    );
  };
          ...
          <View style={[layout.narrowWidthStyle, { width: '100%', alignItems: 'center' }]}>
            ...
            <Pressable
              ...
              style={[
                ...
                { backgroundColor: info.color, opacity: nicknameText.trim() ? 1 : 0.5 },
              ]}
        ...
        <View style={[layout.maxWidthStyle, { flex: 1, width: '100%' }]}>
          {isLoading ? (
            <View style={styles.loadingState}>
              <ActivityIndicator size="large" color={info.color} />
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <Pressable onLongPress={() => handleReport()}>
                  <MessageBubble
                    message={item}
                    isOwn={item.nickname === nickname}
                    colors={colors}
                    isDark={isDark}
                  />
                </Pressable>
              ...
              contentContainerStyle={[
                styles.messagesList,
                { paddingBottom: 8, paddingHorizontal: layout.horizontalPadding },
              ]}
              ...
              ListEmptyComponent={
                <View style={styles.emptyChat}>
                  ...
                  >
                    <Ionicons name="chatbubble-ellipses-outline" size={36} color={info.color} />
                  </LinearGradient>
                  <Text style={[styles.emptyChatTitle, { color: colors.text }]}>
                    {channel === 'prayer' ? 'Share a Prayer Request' : 'Start the Conversation'}
                  </Text>
                  <Text style={[styles.emptyChatText, { color: colors.textSecondary }]}>
                    {channel === 'prayer'
                      ? 'Your prayer requests are seen and prayed over by the community.'
          ...
          >
            <TextInput
              ...
              placeholder={
                channel === 'prayer' ? 'Share a prayer request...' : 'Type a message...'
              }
              placeholderTextColor={colors.textSecondary}
              value={inputText}
...
const styles = StyleSheet.create({
  ...
  loadingText: { fontSize: 13, fontFamily: fontFamily.medium },
  bubbleContainer: {
    flexDirection: 'row',
    marginBottom: 10,
    alignItems: 'flex-end',
  ...
  emptyChatTitle: { fontSize: 17, fontFamily: fontFamily.bold, textAlign: 'center' },
  emptyChatText: {
    fontSize: 13,
    fontFamily: fontFamily.regular,
    textAlign: 'center',
